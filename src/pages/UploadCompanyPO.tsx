...
import { Upload, FileText, Eye, Plus, Building2 } from "lucide-react";
...
const UploadCompanyPO = () => {
  ...
  const { data: existingCompanyPOs } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("company_pos").select("customer_po_id");
      if (error) throw error;
      return data;
    },
  });

  const availableCustomerPOs = customerPOs?.filter(cpo => !existingCompanyPOs?.some(companyPO => companyPO.customer_po_id === cpo.id));
  const selectedCPO = customerPOs?.find(po => po.id === selectedCustomerPO);
  ...
  const uploadCompanyPOMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error: uploadError } = await supabase.storage.from("documents").upload(filePath, companyPOFile);
      if (uploadError) throw uploadError;

      const { data: insertedCompanyPO, error: insertError } = await supabase
        .from("company_pos")
        .insert({ customer_po_id: selectedCustomerPO, po_number: companyPONumber, distributor_name: distributorName, uploaded_by: user.id, file_path: filePath, file_name: companyPOFile.name })
      ...
      if (quoteId) {
        ...
        if (needsProject) {
          ...
          const { data: projectData, error: projectError } = await supabase.functions.invoke('create-project-from-order', { body: { customerName, quoteNumber } });
          if (!projectError && projectData?.project) projectId = projectData.project.id;
        }
        await supabase.from("workflow_tracker").update({ company_po_id: insertedCompanyPO.id, current_stage: "company_po_uploaded", project_id: projectId }).eq("quote_id", quoteId);
        ...
        if (ordersUsers && ordersUsers.length > 0) {
          const notifications = ordersUsers.map(u => ({
            user_id: u.user_id, title: "Company PO Created - Awaiting Waybill",
            message: `Company PO created for quote ${relatedCPO?.quotes?.quote_number}. Please create waybill.`,
            related_type: "workflow", related_id: quoteId,
          }));
    ...
    onSuccess: () => {
      ...
      setSelectedCustomerPO(""); setCompanyPONumber(""); setDistributorName(""); setCompanyPOFile(null); setNeedsProject(false);
    },
    onError: (error: any) => {
      toast({ variant: "destructive", title: "Error", description: error.message });
    },
  ...
  return (
    <DashboardLayout title="Upload Company PO">
      <div className="max-w-2xl mx-auto">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Upload className="h-5 w-5" /> Upload Company Purchase Order</CardTitle>
            <CardDescription>Link a company PO to a customer purchase order</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label>Select Customer PO</Label>
              <select className="w-full h-10 px-3 py-2 border border-input bg-background rounded-md" value={selectedCustomerPO} onChange={(e) => setSelectedCustomerPO(e.target.value)}>
                <option value="">Select a customer PO...</option>
                {availableCustomerPOs?.map((po: any) => (<option key={po.id} value={po.id}>{po.po_number} - {po.quotes?.customer_name}</option>))}
              </select>
            </div>
            {selectedCPO && (
            ...
            <div className="space-y-2">
              ...
              <Input value={companyPONumber} onChange={(e) => setCompanyPONumber(e.target.value)} placeholder="CPO-2025-001" />
            </div>
            <div className="space-y-2">
              ...
              {distributors && distributors.length > 0 ? (
                <select className="w-full h-10 px-3 py-2 border border-input bg-background rounded-md" value={distributorName} onChange={(e) => setDistributorName(e.target.value)}>
                  <option value="">Select a distributor...</option>
                  {distributors.map((d) => (<option key={d.id} value={d.name}>{d.name}</option>))}
                </select>
              ) : (
                <Input value={distributorName} onChange={(e) => setDistributorName(e.target.value)} placeholder="Distributor Company Name" />
              )}
            </div>
            <div className="space-y-2">
              <Label>Company PO File</Label>
              <Input type="file" accept=".pdf" onChange={(e) => setCompanyPOFile(e.target.files?.[0] || null)} />
            </div>
            <div className="flex items-center space-x-2">
              <Checkbox id="needs-project" checked={needsProject} onCheckedChange={(checked) => setNeedsProject(checked as boolean)} />
              <Label htmlFor="needs-project" className="text-sm">This order requires project implementation</Label>
            </div>
            <Button onClick={() => uploadCompanyPOMutation.mutate()} disabled={!selectedCustomerPO || !companyPONumber || !distributorName || !companyPOFile || uploadCompanyPOMutation.isPending} className="w-full">
      ...
      <Dialog open={showAddDistributor} onOpenChange={setShowAddDistributor}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Building2 className="h-5 w-5" /> Add New Distributor</DialogTitle>
          </DialogHeader>
